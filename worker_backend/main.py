...
async def _scheduler_loop():
    interval = float(os.environ.get("SCHEDULER_INTERVAL_SECONDS", "5"))
    while True:
        try:
...
FRONTEND_DIR = Path(__file__).parent.parent / "worker-frontend" / "dist"
...
@app.get("/api/proxies", response_model=list[ProxyEndpointResponse])
async def list_proxies():
    return [ProxyEndpointResponse(**proxy) for proxy in db.list_proxy_endpoints()]


@app.post("/api/proxies", response_model=ProxyEndpointResponse, status_code=201)
async def create_proxy(req: ProxyEndpointCreate):
    try:
        proxy = db.create_proxy_endpoint(**req.model_dump())
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc))
    except Exception as exc:
        logger.warning("Failed to create proxy endpoint: %s", exc)
        raise HTTPException(status_code=409, detail="Proxy endpoint already exists")
    return ProxyEndpointResponse(**proxy)


@app.post("/api/proxies/import", response_model=ProxyImportResponse)
async def import_proxies(req: ProxyImportRequest):
    created, errors = db.import_proxy_csv(req.csv)
    return ProxyImportResponse(created=created, errors=errors)
...
async def list_tasks():
    return [TaskResponse(**task) for task in db.list_tasks()]


@app.post("/api/tasks", response_model=TaskResponse, status_code=201)
async def create_task(req: TaskCreate):
    try:
        task = scheduler.submit_task(req.model_dump())
    except scheduler.PolicyDenied as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    except KeyError:
        raise HTTPException(status_code=404, detail="Profile not found")
...
async def list_runs():
    return [RunResponse(**run) for run in db.list_profile_runs()]
...
async def scheduler_tick():
    try:
        status = await scheduler.tick(browser_mgr)
    except Exception as exc:
