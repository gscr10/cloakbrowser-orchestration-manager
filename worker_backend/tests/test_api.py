...
def test_create_and_list_proxy(app_client: TestClient):
    resp = app_client.post("/api/proxies", json={
        "protocol": "http",
    ...
    assert resp.status_code == 201
    data = resp.json()
    assert data["host"] == "proxy.example.test"
    assert "password" not in data

    listed = app_client.get("/api/proxies")
    assert listed.status_code == 200
    assert listed.json()[0]["host"] == "proxy.example.test"


def test_import_proxies(app_client: TestClient):
    resp = app_client.post("/api/proxies/import", json={
        "csv": "protocol,host,port\nhttp,proxy.example.test,8080\nhttp,,9999\n"
    })
    assert resp.status_code == 200
    data = resp.json()
...
def test_scheduler_tick_launches_queued_task(app_client: TestClient):
    ...
    proxy = app_client.post("/api/proxies", json={
        "protocol": "http",
        "host": "proxy.example.test",
        "port": 8080,
    }).json()
    ...
    resp = app_client.post("/api/scheduler/tick")
    assert resp.status_code == 200
    assert resp.json()["queued_count"] == 0
    launched_profile = main.browser_mgr.launch.call_args.args[0]
    assert launched_profile["id"] == profile["id"]
    assert launched_profile["proxy"] == f"http://{proxy['host']}:{proxy['port']}"
...
def test_set_clipboard_success(app_client: TestClient):
    ...
    with patch("worker_backend.main.asyncio.create_subprocess_exec", new_callable=AsyncMock, return_value=mock_proc):
