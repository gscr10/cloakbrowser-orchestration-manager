...
from worker_backend.browser_manager import (
...
import worker_backend.runtime_limits as runtime_limits
from worker_backend.runtime_limits import launch_block_reason, max_running_profiles
...
async def test_launch_rejects_when_resource_pressure(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.delenv("MAX_RUNNING_PROFILES", raising=False)
    monkeypatch.setattr("worker_backend.browser_manager.launch_block_reason", lambda: "low memory")
    mgr = BrowserManager()
