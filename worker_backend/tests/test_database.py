...
def test_init_db_creates_tables(tmp_db: Path):
    ...
    assert "proxy_endpoints" in names
    assert "tasks" in names
    assert "profile_runs" in names
...
# ── proxy endpoints / tasks ─────────────────────────────────────────────────


def test_create_proxy_endpoint_redacts_password(tmp_db: Path):
    proxy = db.create_proxy_endpoint(
        protocol="http",
    ...
    assert proxy["name"] == "http://proxy.example.test:8080"
    assert proxy["password"] == "********"
    assert proxy["tags"] == ["residential"]


def test_import_proxy_csv_collects_errors(tmp_db: Path):
    created, errors = db.import_proxy_csv(
        "protocol,host,port,username,password,tags\n"
        "socks5,proxy.example.test,1080,user,secret,eu\n"
        "http,,8080,,,\n"
    )
    assert len(created) == 1
    assert created[0]["protocol"] == "socks5"
    assert errors == [{"line": 3, "error": "host is required"}]


def test_create_and_update_task(tmp_db: Path):
    profile = db.create_profile("Task Profile")
    task = db.create_task(
    ...
    assert db.next_queued_task()["id"] == task["id"]
    updated = db.update_task(task["id"], status="cancelled")
